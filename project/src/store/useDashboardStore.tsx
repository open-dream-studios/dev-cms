...
import { LayoutConfig, ModuleMap, ShapeId, ModuleId, LayoutId, SectionId } from "../types/dashboard";
...
interface DashboardState {
  ...
  findShapeById: (shapeId: ShapeId) => { sectionId: SectionId; shapeIndex: number } | null;
...
export const useDashboardStore = create<DashboardState>((set, get) => ({
  ...
  layout: { id: "empty", name: "empty", sections: [] },
  ...
  updateSection(sectionId, patch) {
    ...
    const sections = layout.sections.map((s) => (s.id === sectionId ? { ...s, ...patch } : s));
  ...
  setModuleInShape(shapeId, moduleId) {
    ...
    const sections = layout.sections.map((s) => {
      const shapes = s.shapes.map((sh) => (sh.id === shapeId ? { ...sh, moduleId } : sh));
      return { ...s, shapes };
    });
    set({ layout: { ...layout, sections } });
  },
  ...
  findShapeById(shapeId) {
    ...
    for (const s of layout.sections) {
      ...
      if (idx >= 0) return { sectionId: s.id, shapeIndex: idx };
    }
    return null;
  },
