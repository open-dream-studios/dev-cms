...
const serviceAccount: Record<string, any> = {};
...
export const getSheetData = async () => {
  ...
  //     version: "v4",
  //     auth: client as sheets_v4.Options["auth"],
  //   });
...
export async function getSheetSharedEmails() {
  ...
  if (!res.data.permissions) return [];
...
export async function updateRow(rowIndex: number, rowData: any) {
  const client = await auth.getClient();
  const sheets = google.sheets({
    version: "v4",
    auth: client as sheets_v4.Options["auth"],
...
export async function setNotes(row: number, value: number) {
  const client = await auth.getClient();
  const sheets = google.sheets({
    version: "v4",
    auth: client as sheets_v4.Options["auth"],
