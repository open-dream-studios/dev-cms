// server/handlers/definitions/moduleHandlers.ts
import {
  autoCompleteAddress,
  addressDetails,
...
> = {
  "customer-products-google-sheets-module": async (
    ...
    try {
      ...
      let spreadsheetId: any = configKeys.find(
        (key) => key.integration_key === "sheetId"
      );
      let sheetName: any = configKeys.find(
        (key) => key.integration_key === "sheetName"
      );
      let serviceAccountJson: any = configKeys.find(
  ...
  "customer-products-wix-sync-module": async (
    ...
    try {
      ...
      let WIX_GENERATED_SECRET: any = configKeys.find(
        (key) => key.integration_key === "WIX_GENERATED_SECRET"
      );
      let WIX_BACKEND_URL: any = configKeys.find(
  ...
  "google-maps-api-module": async (
    ...
    try {
      ...
      let GOOGLE_API_KEY: any = configKeys.find(
