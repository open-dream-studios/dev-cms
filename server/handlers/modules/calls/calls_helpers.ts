...
export function normalizeUSNumber(num: any) {
  if (!num) return "";
...
export function mulawDecode(muLawByte: any) {
...
export function decodeMuLawBuffer(muLawBuffer: any) {
...
export async function flushStreamBuffer(sid: any) {
  // const s = callBuffers[sid];
  // if (!s || !s.chunks.length) return;
