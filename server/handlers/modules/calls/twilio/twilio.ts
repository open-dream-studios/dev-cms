...
export const callBuffers: Record<string, any> = {}; // streamSid -> { chunks:[], track }
 ...
 */
export function handleTwilioStream(wss: any) {
    ...
    ws.on("message", async (msg: any) => {
      ...
      if (data.event === "stop") {
        ...
        if (foundProjectId !== null) {
          ...
          if (info) {
            info.streams = info.streams.filter((s: any) => s !== streamSid);
            info.childSids = info.childSids.filter((s: any) => s !== callSid);
