...
export async function getProjectByNumber(phoneNumber: any) {
...
export async function getTwilioKeys(projectId: any) {
  const q = `
  ...
  `;
  try {
    const [rows] = await db.promise().query<RowDataPacket[]>(q, [projectId]);
